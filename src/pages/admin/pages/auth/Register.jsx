...
const Register = () => {
  ...
  var maxlength = /^[a-zA-Z0-9!@#\$%\^\&*\)\(+=._-]{6,}$/g;
  // handel register user
  const handleSubmit = async () => {
    try {
      if (maxlength.test(form.password) === false) {
        return toast.warn(
          "Password must include one uppercase, special character and more than 6 character"
        );
      } else {
        ...
        if (res.data.success) {
          navigate("/login");
          return toast.success("User register sucessfully.");
        }
      }
    } catch (error) {
